...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `codeforge` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@codeforge/shared/git`, `@codeforge/shared/DrainableWorker`) — no barrel index.
