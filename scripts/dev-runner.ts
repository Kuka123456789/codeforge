...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("CODEFORGE_PORT_OFFSET"),
  devInstance: optionalStringConfig("CODEFORGE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid CODEFORGE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `CODEFORGE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric CODEFORGE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed CODEFORGE_DEV_INSTANCE=${seed}` };
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly codeforgeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(codeforgeHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      CODEFORGE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.CODEFORGE_PORT = String(serverPort);
      output.VITE_WS_URL = `ws://localhost:${serverPort}`;
    } else {
      delete output.CODEFORGE_PORT;
      delete output.VITE_WS_URL;
      delete output.CODEFORGE_AUTH_TOKEN;
      delete output.CODEFORGE_MODE;
      delete output.CODEFORGE_NO_BROWSER;
      delete output.CODEFORGE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.CODEFORGE_HOST = host;
    }

    if (!isDesktopMode && authToken !== undefined) {
      output.CODEFORGE_AUTH_TOKEN = authToken;
    } else if (!isDesktopMode) {
      delete output.CODEFORGE_AUTH_TOKEN;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.CODEFORGE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.CODEFORGE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.CODEFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.CODEFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.CODEFORGE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.CODEFORGE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.CODEFORGE_MODE = "web";
      delete output.CODEFORGE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read CODEFORGE_PORT_OFFSET/CODEFORGE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("CODEFORGE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "CODEFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("CODEFORGE_LOG_WS_EVENTS"),
    };
    ...
    const env = yield* createDevRunnerEnv({
      ...
      serverOffset,
      webOffset,
      codeforgeHome: input.codeforgeHome,
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.CODEFORGE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.CODEFORGE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  codeforgeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all CodeForge data (equivalent to CODEFORGE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("CODEFORGE_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to CODEFORGE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("CODEFORGE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to CODEFORGE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("CODEFORGE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to CODEFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("CODEFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to CODEFORGE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("CODEFORGE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to CODEFORGE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("CODEFORGE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to CODEFORGE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("CODEFORGE_PORT")),
  ),
