...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit CODEFORGE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "CODEFORGE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid CODEFORGE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults CODEFORGE_HOME to ~/.codeforge when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CODEFORGE_HOME, resolve(homedir(), ".codeforge"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          codeforgeHome: "/tmp/custom-codeforge",
          authToken: "secret",
          noBrowser: true,
        ...
        assert.equal(env.CODEFORGE_HOME, resolve("/tmp/custom-codeforge"));
        assert.equal(env.CODEFORGE_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.CODEFORGE_NO_BROWSER, "1");
        assert.equal(env.CODEFORGE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.CODEFORGE_LOG_WS_EVENTS, "1");
        assert.equal(env.CODEFORGE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            CODEFORGE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          codeforgeHome: undefined,
        ...
        assert.equal(env.CODEFORGE_MODE, "web");
        assert.equal(env.CODEFORGE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CODEFORGE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom codeforgeHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
          serverOffset: 0,
          webOffset: 0,
          codeforgeHome: "/tmp/my-codeforge",
          authToken: undefined,
          noBrowser: undefined,
          autoBootstrapProjectFromCwd: undefined,
          logWebSocketEvents: undefined,
        ...
        assert.equal(env.CODEFORGE_HOME, resolve("/tmp/my-codeforge"));
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            CODEFORGE_PORT: "3773",
            CODEFORGE_AUTH_TOKEN: "stale-token",
            CODEFORGE_MODE: "web",
            CODEFORGE_NO_BROWSER: "0",
            CODEFORGE_HOST: "0.0.0.0",
        ...
        assert.equal(env.CODEFORGE_PORT, undefined);
        assert.equal(env.CODEFORGE_AUTH_TOKEN, undefined);
        assert.equal(env.CODEFORGE_MODE, undefined);
        assert.equal(env.CODEFORGE_NO_BROWSER, undefined);
        assert.equal(env.CODEFORGE_HOST, undefined);
